...
const createGroupChat = AsyncHandler(async(req,res)=>{
     if(!req.body.name||!req.body.users){
          res.status(400).json({success:false,message:"Filled the required credentials"})
     }
     ...
     if(users.length < 2){
          return res.status(400).json({success:false,message:"Minimum 2 users are required for the group"})
     }
     users.push(req.user);
     // console.log(users);
     const groupChat =await Chat.create({chatName:req.body.name,
          isGroupChat:true,
          users,
          groupAdmin:req.user
...
const rename = AsyncHandler(async(req,res)=>{
     ...
     try {
          ...
          const update = await Chat.findByIdAndUpdate(chatId,{chatName},{new:true})
...
try{
     ...
     res.status(200).json({success:true,message:"added Successfully",added})
} catch (error) {
     res.status(400).json({success:false,message:"Internal Server Error"})
}
...
try{
     ...
     res.status(200).json({success:true,message:"removed Successfully",remove})
} catch (error) {
     res.status(400).json({success:false,message:"Internal Server Error"})
}
...
module.exports = {accessChat,fetchChat,createGroupChat,rename,addGroup,removeGroup};